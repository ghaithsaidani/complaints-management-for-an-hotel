...
import {OtDetailsComponent} from "../ot-details/ot-details.component";
...
@Component({
  selector: 'app-consult-ots',
  templateUrl: './consult-ots.component.html',
  styleUrls: ['./consult-ots.component.scss']
})
export class ConsultOtsComponent implements OnInit {
  ...
  /*ot:any*/
  displayedColumns: string[] = ['num_ot','type_ot','temps','cloturage','etat-change','settings'];
  hidded=true
  all = false
  blocked = false
  active = true
  ...
  constructor(private cd:ChangeDetectorRef,private OtService:MyserviceService,private dialog:MatDialog,private concerned:ConcernedService) {
  ...
  getOt() {
    ...
    this.OtService.Get("/Ot/tous").subscribe(data=>{
        this.Ot.data=data
        console.log(this.Ot.data[0])
    })
  ...
  openOtDetailsDialog(ot:OrdreTravail){
    this.OtService.getDetails(ot.id,"/Ot/chercherParId/").subscribe(data=>{
      ...
      const dialogRef = this.dialog.open(OtDetailsComponent,{autoFocus: false,panelClass: 'Components-details-container'});
      this.concerned.setDialogId(dialogRef.id)
      dialogRef.afterClosed().subscribe(()=> {
        this.getOt()
      })
  ...
  InstanceofOtPreventif(element:OtPreventif){
  ...
  TempsEstime(element:any){
    ...
    else if(this.InstanceofOtPreventif(element)){
      element.preventifR.map((ligneOt:LigneOtPreventif)=>{
        periodsum.add(ligneOt.tempsEstime)})
      if(moment.duration(periodsum,'minutes').hours()==0){
        return moment.duration(periodsum, 'minutes').minutes() + " minutes";
      }
  ...
  openDialog(ot:OrdreTravail){
    ...
    const dialogRef = this.dialog.open(DialogComponent,{data:{title:"Ordres de Travail",content: content},autoFocus: false,panelClass: 'choice-dialog-container'});
