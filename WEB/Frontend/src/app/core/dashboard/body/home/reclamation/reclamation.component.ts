...
export class ReclamationComponent implements OnInit,AfterViewInit {
  ...
  displayedColumns: string[] = ['designation','type','priorite','settings'];
  ...
  minute!:boolean
  second!:boolean
  constructor(private router:Router,private cd:ChangeDetectorRef,private reclamationsService:MyserviceService,private concernedReclamation:ConcernedService,private _liveAnnouncer: LiveAnnouncer,private dialog:MatDialog) { }
  ...
  getReclamations() {
    ...
    this.reclamationsService.Get("/reclamations/non-traite").subscribe(data=>{
